...
class SrhFitsFile():
    ...
    def normalizeFlux(self):
        zerosFits = fits.open('srh_1224_cp_zeros_20230806.fits')
        corrZeros = zerosFits[2].data['corrI']
        fluxZeros = zerosFits[2].data['fluxI']

        fluxNormFits = fits.open('srh_1224_cp_fluxNorm_20230806.fits')
        fluxNormI = fluxNormFits[2].data['fluxNormI']
            
        
        self.antAmp_Lcp = NP.sqrt(NP.abs(self.ampLcp_c[:,:,self.antennaA]) * NP.abs(self.ampLcp_c[:,:,self.antennaB]))
        self.antAmp_Rcp = NP.sqrt(NP.abs(self.ampRcp_c[:,:,self.antennaA]) * NP.abs(self.ampRcp_c[:,:,self.antennaB]))
    ...
    def loadGains(self, filename):
        ...
        try:
            self.nAntPhaLcp = NP.array(currentGains['sPhaseLcp'])
            self.nAntPhaRcp = NP.array(currentGains['sPhaseRcp'])
            self.nAntAmpLcp = NP.array(currentGains['sAmpLcp'])
            self.nAntAmpRcp = NP.array(currentGains['sAmpRcp'])
        except:
            pass
        try:
    ...
    def solarPhase(self, freq):
        ...
        u,v,w = base2uvw_1224.base2uvw(self.hAngle, self.RAO.declination, antAInd, antBInd)
...
#            sunVisPhases = NP.array((NP.pi, NP.pi))
    ...
    def vis2uv(self, scan, phaseCorrect = True, amplitudeCorrect = False, PSF=False, average = 0):
        ...
        for i in range(len(self.antZeroRow)):
            ...
            if not (NP.any(self.flags_ew == i) or NP.any(self.flags_ew == self.center_ant)):
