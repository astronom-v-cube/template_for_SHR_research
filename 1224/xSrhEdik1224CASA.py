...
#        self.srhFits.changeNorthPhase(self.nLcpPhaseCorrection, self.nRcpPhaseCorrection)
    ...
    def showImage(self):
        ...
        if self.indexOfImageType == 0 or self.indexOfImageType == 4 or self.indexOfImageType == 5:
            if self.autoscaleButton.isChecked():
                ...
                self.rcpCanvas.imshow(self.rcpData, NP.min(self.lcpData), NP.max(self.lcpData))
            else:
    ...
    def onCalculatePhase(self):
        self.srhFits.calculatePhaseCalibration()
        if (self.imageUpdate):
            self.buildImage()
            self.showImage()
            
    def onCalculatePhaseCurrent(self):
        self.srhFits.updateAntennaPhase(self.srhFits.frequencyChannel)
        if (self.imageUpdate):
            self.buildImage()
            self.showImage()
...
#        try:
        ...
        ms2Table = srh1224MS2.Srh1224Ms2(saveName)
